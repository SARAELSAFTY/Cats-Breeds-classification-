...
class CatBreedGUI:
    ...
    def create_widgets(self):
        ...
        image_frame.pack(pady=20, padx=50, fill=tk.BOTH, expand=True)
        ...
        pred_frame.pack(pady=20, padx=50, fill=tk.X)
            ...
            if os.path.exists(model_file):
                try:
                    ...
                    self.model_status.config(
                        text=f"✅ Model loaded: {model_file}",
                        fg='#27ae60'
                    )
                    return
    ...
    def predict_breed(self):
        ...
        try:
            ...
            self.main_pred_label.config(
                text=f"🐱 {predicted_class.replace('_', ' ')}"
            )
